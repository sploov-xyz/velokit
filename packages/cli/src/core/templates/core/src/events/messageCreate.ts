...
export default async (client: SploovClient, message: Message) => {
    ...
    if (command.userPermissions) {
        const missing = message.member?.permissions.missing(command.userPermissions as PermissionResolvable[]);
    ...
    if (command.botPermissions) {
        const missing = message.guild?.members.me?.permissions.missing(command.botPermissions as PermissionResolvable[]);
        if (missing && missing.length > 0) {
